...
def fct_test(X, x) :
	return X[0]*np.cos(2*np.pi*x + X[3]) + X[1]*np.cos(2*np.pi*x/365 + X[4]) + X[2] + X[5]*np.cos(4*np.pi*x/365 + X[6])
...
X = np.array([2.0,10.0,285.0,0.0,0.0,0.0, 0.0])
...
X = np.array([30.0,10.0,75,0.0,0.0,0.0,0.0])
# A,B,C,phi1,phi2
