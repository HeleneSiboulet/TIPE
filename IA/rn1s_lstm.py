...
class Net(nn.Module) :
    ...
    def __init__(self) :
        ...
        self.lstm = nn.LSTM(7, HIDDEN, 3).double()
        self.fc1 = nn.Linear(HIDDEN, HIDDEN).double()
        self.fc2 = nn.Linear(HIDDEN, HIDDEN//2).double()
        self.fc3 = nn.Linear(HIDDEN//2, 2*longueur_prevision).double()
...
for tour in range(nb_tour_apprentissage) :
	...
	for j in range(20) :
		...
		batch_source.append(temperature_continu[i-longueur_apprentissage:i,:].unsqueeze(1))
		...
		try :
			...
			batch_target = target
	source = torch.cat(batch_source,1)
	target = batch_target
	#print(source.size())
	out = net(source)
...
for tour in range(nb_tour) :
	...
	entre = torch.tensor(entre).double().unsqueeze(1)
...
torch.save(net.state_dict(), "./net1s_lstm.nn")
