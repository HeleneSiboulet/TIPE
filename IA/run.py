...
class Net(nn.Module) :
    ...
    def __init__(self) :
        ...
        self.fc1 = nn.Linear(224, 56).double()
        self.fc2 = nn.Linear(56, 56).double()
        self.fc3 = nn.Linear(56, 8).double()
...
for tour in range(1000) :
	...
	for j in range(20) :
		...
		batch_source.append(torch.tensor(temperature_continu[i-longueur:i]).double().view(1,-1))
		target = []
...
for i in range(0,taille,8) :
	...
	for j in range(56) :
		an = (2019.0 - ma)/sta
		cdt,sdt = convertiseur_date(temperature_test[0]["2019"][(j+i)%taille])
