...
def fct_test(X, x, annee) :
	return X[0]*np.cos(2*np.pi*x + X[3]) + X[1]*np.cos(2*np.pi*x/365 + X[4]) + X[2] + X[5] * (int (annee) - 1996)
...
X = np.array([2.0,10.0,285.0,0.0,0.0, 0.0])
# A,B,C,D,phi1,phi2,phi3, E
# Acos(w1t + phi1) + Bcos(w2t+phi2) + Ccos(w3t+phi3) + D + E*annee
#w1 jour w2 annee w3 demi-journee

for i in range(100) :
	val_actuel = []
	for anne in temperature[0]. keys () :
		for date in dates :
			val_actuel.append(fct_test(X, date, annee))
		val_actuel_ecart += ecart_temperature([dates, val_actuel, annee])
	if i == 0 :
	...
	for j in range(len(X)) :
		...
		for anne in temperature[0].keys ():
			for date in dates :
				val_modif.append(fct_test(X_modif,date, annee ))
